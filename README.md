...
- **🚀 高性能**：基于 Candle 框架，支持 GPU 加速（CUDA/Metal）。跟candle有关，跟本项目无关。
- **🔧 完整流程**：包含分词器训练、模型训练、推理等完整工具链。只实现，不保证完整。
- **📚 教育友好**：代码结构清晰，适合学习 Transformer 架构。还行吧，kvcache没那么清晰，但是也不改了，让cursor生成解释来看吧。
- **🎯 轻量级**：专注核心功能，代码量适中，易于理解和修改。让cursor来给你解释。
